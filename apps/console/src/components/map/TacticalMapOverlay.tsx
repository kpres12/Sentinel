...
export interface TacticalMapOverlayProps {
  onToggle: () => void
}

export function TacticalMapOverlay({ onToggle }: TacticalMapOverlayProps) {
  ...
  const tools = [
    { id: 'target', icon: Target, label: 'TARGET', color: 'text-fire-400', active: true },
    { id: 'wind', icon: Wind, label: 'WIND', color: 'text-tactical-400', active: false },
    { id: 'flame', icon: Flame, label: 'FIRE', color: 'text-fire-500', active: false },
    { id: 'radio', icon: Radio, label: 'COMMS', color: 'text-tactical-400', active: false },
    { id: 'camera', icon: Camera, label: 'CAM', color: 'text-tactical-400', active: false },
    { id: 'navigation', icon: Navigation, label: 'NAV', color: 'text-tactical-400', active: false },
  ]
  ...
  const layerControls = [
    { id: 'telemetry', label: 'TELEMETRY', active: true, color: 'text-tacticalGreen-400' },
    { id: 'detections', label: 'DETECTIONS', active: true, color: 'text-fire-400' },
    { id: 'risk', label: 'RISK HEATMAP', active: false, color: 'text-warning-400' },
    { id: 'perimeter', label: 'PERIMETER', active: false, color: 'text-fire-500' },
    { id: 'wind', label: 'WIND VECTORS', active: false, color: 'text-tactical-400' },
    { id: 'terrain', label: 'TERRAIN', active: true, color: 'text-tactical-400' },
  ]
      ...
      {showCompass && (
        <div className="absolute top-4 right-4 w-24 h-24 pointer-events-auto">
          <div className="relative w-full h-full">
            <div className="absolute inset-0 border-2 border-tactical-400 rounded-full bg-dark-900/80 backdrop-blur-sm">
      ...
      {showRuler && (
        <div className="absolute bottom-4 left-4 pointer-events-auto">
          <div className="tactical-panel rounded-lg p-3">
            ...
            <div className="w-32 h-1 bg-tactical-400 relative">
        ...
        <div className="absolute bottom-4 right-4 pointer-events-auto">
          <div className="tactical-panel rounded-lg p-4">
            <div className="flex items-center justify-between mb-3">
              <h3 className="tactical-subtitle">LAYERS</h3>
              <button
              ...
              {layerControls.map((layer) => (
                <button
                  ...
                  className={`flex items-center gap-2 w-full p-2 rounded transition-all ${
                    layer.active ? 'bg-tactical-500/20 border border-tactical-500/30' : 'hover:bg-dark-800'
                  }`}
                >
                  <div className={`w-3 h-3 rounded-full ${layer.active ? layer.color.replace('text-', 'bg-') : 'bg-dark-600'}`}></div>
                  <span className="text-sm font-mono text-tactical-300">{layer.label}</span>
                  {layer.active ? <Eye className="w-4 h-4 text-tactical-400" /> : <EyeOff className="w-4 h-4 text-dark-400" />}
                </button>
              ))}
            </div>
          </div>
        ...
        <div className="tactical-panel rounded-lg p-3">
          <div className="flex items-center gap-4">
            <div className="flex items-center gap-2">
              ...
              <span className="text-xs font-mono text-tacticalGreen-400">SYSTEM ONLINE</span>
            </div>
            <div className="flex items-center gap-2">
              <Shield className="w-4 h-4 text-tactical-400" />
              <span className="text-xs font-mono text-tactical-400">SECURE</span>
            </div>
            <div className="flex items-center gap-2">
              <Zap className="w-4 h-4 text-warning-400" />
              <span className="text-xs font-mono text-warning-400">3 ALERTS</span>
            </div>
          </div>
        </div>
      </div>
